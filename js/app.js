...
function updateCart() {
    ...
    cart.forEach((item, idx) => {
        ...
        totalItems += item.qty;
        // Soportar size como objeto o string
        const displaySize = item.size ? (typeof item.size === 'string' ? item.size : (item.size.name || '')) : '';
...
function addToCart(id, qty = 1, sizeIndex = null) {
    ...
    if (availableSizes.length > 0) {
        ...
        // comparar tolerando size como string u objeto
        const existingInCart = cart.find(i => {
            if (!i.size) return false;
            const nameA = typeof i.size === 'string' ? i.size : (i.size.name || '');
            return i.id === id && String(nameA).toLowerCase() === String(sizeObj.name).toLowerCase();
        });
...
finalizeBtn && finalizeBtn.addEventListener('click', async () => {
    ...
    const items = cart.map(i => ({
        ...
        qty: i.qty,
        price: i.price,
        size: i.size ? (typeof i.size === 'string' ? i.size : i.size.name) : null
    }));
